    ...
    constructor() {
        ...
        if (apiKey) {
            try {
                ...
                this.logger.error('Failed to initialize Google Generative AI client', error);
            }
        } else {
            this.logger.warn('GOOGLE_API_KEY not set. AI Summarization will fall back to mock response.');
        }
    }

    async summarize(text: string): Promise<string | undefined> {
        if (!this.isConfigured) {
            this.logger.warn('Google Generative AI not configured. Returning mock summary.');
